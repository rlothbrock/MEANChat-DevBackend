...
function dispatcher(err_, res){
    ...
    else if(err_.name === 'ValidationError') error = valErrHandler(err_);
    else if(err_.code === 11000 ) error = mongoErrHandler(err_);
    else if(err_.name === 'JsonWebTokenError') error = jwtErrHandler();
    else if(err_.name === 'TokenExpiredError') error = jwtExpiredErrHandler();
